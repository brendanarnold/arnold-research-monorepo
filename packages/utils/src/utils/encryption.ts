...
export const encryptAes = (value: string) => {
  ...
  const cipher = crypto.createCipheriv(encryptionMethod, config.MASTER_AES_KEY, iv)
...
export const decryptAes = (encryptedValue: Uint8Array, iv: Uint8Array, encryptionMethod: string) => {
  const decipher = crypto.createDecipheriv(encryptionMethod, config.MASTER_AES_KEY, iv)
